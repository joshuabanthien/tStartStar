...
tStartStarHists::tStartStarHists(Context & ctx, const string & dirname): Hists(ctx, dirname){
 ...
 // photon
 book<TH1F>("N_gamma", "N^{#gamma}", 10, 0, 10);
 book<TH1F>("pt_gamma", "p_{T}^{#gamma} [GeV/c]", 40, 0, 200);
 book<TH1F>("eta_gamma", "#eta^{#gamma}", 40, -2.1, 2.1);
 book<TH1F>("reliso_gamma", "#gamma Iso", 40, 0, 0.5);
...
void tStartStarHists::fill(const Event & event){
  ...
  for (const Photon & thisgamma : *event.photons){
      ...
      hist("reliso_gamma")->Fill(thisgamma.photonIso(), weight);
  }
